...
type Values = { sector?: string; language?: string; maxRate?: string };
 ...
 */
export async function ExpertDirectoryFilters({ locale, values }: Props) {
  ...
  return (
    <form
      method="get"
      className="grid grid-cols-1 gap-3 rounded-2xl border border-[var(--color-border)] bg-white p-4 sm:grid-cols-2 lg:grid-cols-4"
    >
      <label className="text-xs font-medium text-gray-600">
        {t("filterSector")}
        <select name="sector" defaultValue={values.sector ?? ""} className={`mt-1 ${fieldCls}`}>
      ...
      <label className="text-xs font-medium text-gray-600">
        {t("filterLanguage")}
        <select name="language" defaultValue={values.language ?? ""} className={`mt-1 ${fieldCls}`}>
          <option value="">{t("any")}</option>
